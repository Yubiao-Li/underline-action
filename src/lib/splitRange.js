...
export function splitRange(nativeRange) {
  ...
  const rowBottom = getCharBottom(startContainer, startOffset);
  // 头尾高度一致说明在同一行
  if (rowBottom === getCharBottom(endContainer, endOffset - 1)) {
    range.setStart(startContainer, startOffset);
    range.setEnd(endContainer, endOffset);
    return [range];
  } else {
    const last = findRowLastChar(rowBottom, startContainer, startOffset, endContainer, endOffset);
    range.setStart(startContainer, startOffset);
...
function findRowLastCharSameNode(bottom, node, start, end) {
  if (end - start === 1) {
    return start;
  }
...
function findRowLastChar(top, startContainer, startOffset, endContainer, endOffset) {
  if (startContainer === endContainer) {
    ...
    return {
      node: startContainer,
      offset: findRowLastCharSameNode(
        top,
        startContainer,
        startOffset,
...
function getCharBottom(node, offset) {
